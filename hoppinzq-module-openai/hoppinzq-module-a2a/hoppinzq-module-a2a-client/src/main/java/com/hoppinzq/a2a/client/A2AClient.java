...
public class A2AClient {
     ...
     * 获取AgentCard列表
     * <p>
     * 通过HTTP GET请求从指定的well-known端点获取AgentCard信息。
     * 请求的URI由baseUrl和"/.well-known/agent-card"拼接而成，
     * 接受JSON格式的响应。
     *
     * @return 包含AgentCard信息的列表
     * @throws A2AClientException 当HTTP响应状态码非200时抛出，
     *                            或发生IO错误、线程中断时抛出
     */
    public List<AgentCard> getAgentCards() throws A2AClientException {
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(baseUrl + "/.well-known/agent-cards"))
            ...
            if (response.statusCode() != 200) {
                throw new A2AClientException("HTTP " + response.statusCode() + ": " +
                        (response.body() != null ? response.body() : "empty response"));
            }
            ...
            return objectMapper.readValue(response.body(),
                    objectMapper.getTypeFactory().constructCollectionType(List.class, AgentCard.class));

        } catch (IOException e) {
            throw new A2AClientException("Failed to get agent cards due to IO error", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new A2AClientException("Failed to get agent cards due to interruption", e);
        }
    }
