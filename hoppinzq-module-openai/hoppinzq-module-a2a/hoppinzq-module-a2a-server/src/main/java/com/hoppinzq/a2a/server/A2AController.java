...
//    }
    ...
    public ResponseEntity<JSONRPCResponse> handleJsonRpcRequest(@RequestBody JSONRPCRequest request) {
        ...
        JSONRPCResponse response = switch (request.method()) {
            case "tasks/send" -> server.get(0).handleTaskSend(request);
            case "tasks/get" -> server.get(0).handleTaskGet(request);
            case "tasks/cancel" -> server.get(0).handleTaskCancel(request);
            default -> {
                JSONRPCError error = new JSONRPCError(
    ...
    public SseEmitter handleStreamingTask(@RequestBody JSONRPCRequest request) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                // Process task
                JSONRPCResponse taskResponse = server.get(0).handleTaskSend(request);
    ...
    public ResponseEntity<AgentCard> getAgentCard() {
        return ResponseEntity.ok(server.get(0).getAgentCard());
    }

    @GetMapping("/.well-known/agent-cards")
    public ResponseEntity<List<AgentCard>> getAgentCards() {
        return ResponseEntity.ok(server.stream().map(A2AServer::getAgentCard).toList());
    }
