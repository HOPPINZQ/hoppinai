...
public class SearchBlogServiceEmbeddingImpl implements SearchService {
    ...
    @Override
    @ApiMapping(value = "initBlog2", title = "初始化", description = "初始化博客")
    public void init() {
        ...
        try {
            ...
            for (BlogPO blogPO : blogPOS) {
                blogPO.setText(blogPO.getTitle().length() > 6000 ? blogPO.getTitle().substring(0, 6000) : blogPO.getTitle());
                docList.add(JSON.toJSONString(blogPO));
            }
            OpenAiService service = new OpenAiService(FunctionCallCommon.apiKey, Duration.ofSeconds(60), FunctionCallCommon.openaiProxy);
            for (int i = 0; i < blogPOS.size(); i++) {
                ...
                EmbeddingPO blog = EmbeddingPO.builder()
                        ...
                        .tableId(String.valueOf(blogPOS.get(i).getId()))
                        .tableName("blog")
                        .build();
...
//            embeddingMapper.insertBatch(embeddingPOS);
     ...
     */
    @Override
    @ApiMapping(value = "queryBlog3", title = "搜索博客", description = "搜索博客")
    public List<BlogPO> query(String searchContent) {
        List<EmbeddingPO> embeddingPOS = embeddingMapper.selectList(new LambdaQueryWrapperX<EmbeddingPO>().eq(EmbeddingPO::getTableName, "blog"));
        OpenAiService service = new OpenAiService(FunctionCallCommon.apiKey, Duration.ofSeconds(60), FunctionCallCommon.openaiProxy);
        EmbeddingResult embeddings = service.createEmbeddings(EmbeddingRequest.builder().model(FunctionCallCommon.embedding_model)
        ...
        List<BlogPO> blogPOS = myBlogMapper.selectBatchIds(ids);
