...
public abstract class AbstractAbstractRetryRegisterService extends AbstractRetryTemplate {

    public AbstractAbstractRetryRegisterService() {
    }


    public AbstractAbstractRetryRegisterService(int sleepTime) {
        super.sleepTime = sleepTime < 0 ? 0 : sleepTime;
        this.alwaysRetry = Boolean.TRUE;
    }

    public AbstractAbstractRetryRegisterService(int retryCount, int sleepTime, Boolean alwaysRetry) {
        ...
        for (int i = 0; i < retryCount; i++) {
            try {
                ...
                if (e instanceof RemotingException && e.getMessage().indexOf("java.net.ConnectException") != -1) {
                    ...
                    e.printStackTrace();
                    logger.error("注册服务到注册中心失败,将不再进行注册！请尝试手动注册，或者修改代码！");
                    return false;
                }
