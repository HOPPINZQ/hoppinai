...
/**
 * @author:ZhangQi 重试机制模板，捕获异常将进行重试（你也通过捕获toDo方法返回原方法返回值里的响应码）。
 * 默认10次重试，每分钟尝试执行一次
 **/
public abstract class AbstractRetryTemplate extends TaskTemplate {

    private static final int DEFAULT_RETRY_COUNT = 10;
    protected static Logger logger = LoggerFactory.getLogger(AbstractRetryTemplate.class);
    ...
    public AbstractRetryTemplate(int sleepTime) {
        this.sleepTime = sleepTime;
        this.alwaysRetry = Boolean.TRUE;
    }

    public AbstractRetryTemplate(int retryCount, int sleepTime, Boolean alwaysRetry) {
    ...
    public AbstractRetryTemplate setSleepTime(int sleepTime) {
        this.sleepTime = sleepTime < 0 ? 0 : sleepTime;
        return this;
    }
    ...
    public AbstractRetryTemplate setRetryCount(int retryCount) {
        this.retryCount = retryCount < 0 ? 0 : retryCount;
        return this;
    }
